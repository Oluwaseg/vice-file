...
const roles: { value: PlayerRole; label: string; description: string }[] = [
  {
    value: 'street-racer',
    label: 'Street racer',
    description: 'Fast routes. No witnesses.',
  },
  {
    value: 'nightclub-owner',
    label: 'Nightclub owner',
    description: 'The city talks after midnight.',
  },
      ...
      <main className='identity-screen'>
        ...
        <section className='identity-editor'>
          <ImageEditor
            ...
            options={{
              theme: 'dark',
              features: { imageEditor: { tools: { resize: false } } },
            }}
            onSave={({ dataUrl }) => completeIdentity(dataUrl)}
            onCancel={() => setStep('details')}
            onLoadError={() =>
              setError(
                'Your portrait could not be opened. Choose another image.'
              )
            }
            onError={() =>
              setError(
                'The portrait editor could not start. Refresh and try again.'
              )
            }
          />
        </section>
  ...
  if (step === 'card' && player) {
    return (
      <main className='identity-card-reveal'>
          ...
          <div>
            ...
            <dl>
              <div>
                <dt>Cover</dt>
                <dd>
                  {roles.find((item) => item.value === player.role)?.label}
                </dd>
              </div>
              <div>
                <dt>Base</dt>
                <dd>{player.neighbourhood}</dd>
              </div>
              <div>
                <dt>Status</dt>
                <dd>Active</dd>
        ...
        <Link className='identity-primary-action' href='/dashboard'>
          Enter Vice City
        </Link>
        <button
          className='identity-subtle-action'
          type='button'
    ...
    <main className='identity-screen'>
      ...
      <section className='identity-form' aria-label='Create a cover identity'>
        ...
        <label>
          ...
          >
            <option>Vice Beach</option>
            <option>Downtown</option>
            <option>Little Havana</option>
          </select>
        </label>
        <fieldset>
          <legend>Select your cover</legend>
        ...
        <label className='portrait-upload'>
          ...
          <input
            ...
            onChange={loadPortrait}
          />
          <span>
            {portrait
              ? 'Portrait received. Ready for editing.'
              : 'Choose a PNG, JPEG, or WebP (up to 4 MB).'}
          </span>
        </label>
        {portrait ? (
          <img
            ...
            src={portrait}
            alt='Selected cover portrait preview'
          />
        ) : null}
        {error ? <p className='identity-error'>{error}</p> : null}
        <button
          className='identity-primary-action'
          type='button'
          onClick={beginEditing}
        >
          Edit portrait and create cover
        </button>
      </section>
    </main>
  );
